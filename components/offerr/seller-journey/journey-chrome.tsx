 ...
 *
 * The 16px font size is not a taste decision: iOS Safari zooms the viewport
 * when a text, number, select or textarea control smaller than 16px receives
 * focus. The seller is mid-form when that happens, so the page jumps, the
 * layout reflows, and the field they just tapped moves out from under their
 * thumb. This was measured at 15px across every mobile viewport before the fix.
 *
 * It is a flat 16px rather than a `sm:` step-down to 15px: the one-pixel
 * difference is invisible, and a responsive override here would be a rule that
 * has to stay correct forever to prevent a bug it cannot be observed causing.
 * Body copy elsewhere is untouched — only the controls a seller focuses.
 */
export const inputClass =
  "w-full rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3.5 text-[16px] text-white " +
